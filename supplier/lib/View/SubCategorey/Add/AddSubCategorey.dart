...
class AddSubCategorey extends StatelessWidget {
  AddSubCategorey({this.category});
  Category? category;
  @override
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
        ...
        body: Consumer<AddSubCategoryController>(
          builder: (context, controller, child) => ListView(
            padding: const EdgeInsets.all(8),
            children: [
              Text(
                "إضافة تصنيف فرعي  -  ضمن قسم ${category!.name}",
              ...
              Padding(
                ...
                child: GestureDetector(
                  onTap: () async {
                    ...
                    try {
                      var result =
                          await controller.AddSubCategory(context, category);
                      result.fold(
                        (l) {
                          EasyLoading.dismiss();
