...
class EditeSubCategoreyController with ChangeNotifier {
  TextEditingController namecontroller = TextEditingController();
  SubCategory? subCategory;
  initstate(SubCategory subCategory) {
  ...
  Future<Either<Failure, bool>> EditSubCategory(
      ...
      if (connected) {
        ...
        if (imagesubcat == null) {
          request = await client.requestwithotimage(
            path: AppApi.UpdateSubCategory(subCategory!.id!),
            body: {
          ...
          request = await client.requestimage(
            path: AppApi.UpdateCategory(subCategory!.id!),
            body: {
              'name': namecontroller.text,
            },
        ...
        if (response.statusCode == 200) {
          EasyLoading.dismiss();
          CustomDialog.Dialog(context, title: "تم تعديل التصنيف الفرعي بنجاح");
    ...
    try {
      ...
      if (connected) {
        ...
        response = await client.request(
            requestType: RequestType.GET, path: AppApi.DeleteSubCategory(id));

        if (response.statusCode == 200) {
          EasyLoading.dismiss();
          CustomDialog.Dialog(context, title: "تم حذف التصنيف الفرعي بنجاح");
          CustomRoute.RouteAndRemoveUntilTo(
              context,
              ChangeNotifierProvider<MainPageController>(
                create: (context) => MainPageController()
