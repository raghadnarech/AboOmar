...
class CartController with ChangeNotifier {
  ...
  SelectAppointment(BuildContext context) {
    showTimePicker(initialTime: TimeOfDay.now(), context: context).then((
      value,
    ) {
      ...
      if (connected) {
        ...
        if (response.statusCode == 200) {
          ...
          await pointSystemController.StorePoint(
            ServicesProvider.getphone(),
            'UserOrder',
          );
          CustomRoute.RouteReplacementTo(context, TrackOrder(id: id));
          EmptyCart();
          return Right(true);
  ...
  List<String> generateTimeSlots(
    ...
    while (currentTime.hour < endTime.hour ||
      ...
      currentTime = currentTime.replacing(
        hour:
            currentTime.minute + interval.inMinutes >= 60
                ? currentTime.hour + 1
                : currentTime.hour,
